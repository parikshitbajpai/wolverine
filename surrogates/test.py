...
# parser.add_argument('outputs', type=int, help="Number of output nodes")
...
# Compute Device
gpu = (torch.backends.mps.is_available() or torch.cuda.is_available()) and not args.disable_gpu
...
choice = np.random.choice(range(len(df)), size=(N,), replace=False)
...
X_all, Y_all = torch.tensor(np.asarray(input_df)), torch.tensor(np.asarray(output_df))
